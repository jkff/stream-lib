...
public class SparseShortArray
{
    ...
    public SparseShortArray(int capacity, int itemsPerBlock, int initialBlockCapacity) {
        ...
        this.blocks = new short[capacity/itemsPerBlock+1][];
        ...
        if (p.present) {
            ...
            if (value != 0) {
                ...
                if (size - 1 <= block.length / 4) {
                    // Shrink block 2x
                    short[] newBlock = Arrays.copyOf(block, block.length/2);
            ...
            if (block == null) {
                ...
                block = blocks[whichBlock] = new short[initialBlockCapacity];
            }
            int size = blockSizes[whichBlock];
    ...
    public int bytesUsed() {
        ...
        for (short[] block : blocks) {
            if (block != null) {
                totalBlockSizes += ARRAY_HEADER_SIZE + Short.SIZE * block.length;
            }
        }
