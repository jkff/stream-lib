...
public class SparseByteArray
{
    ...
    public SparseByteArray(int capacity, int itemsPerBlock, int initialBlockCapacity) {
        ...
        this.blocks = new byte[capacity/itemsPerBlock + 1][];
        ...
        if (p.present) {
            ...
            if (value != 0) {
                ...
                if (size - 1 <= block.length / 4) {
                    // Shrink block 2x
                    byte[] newBlock = Arrays.copyOf(block, block.length/2);
            ...
            if (block == null) {
                ...
                block = blocks[whichBlock] = new byte[initialBlockCapacity];
            }
            int size = blockSizes[whichBlock];
