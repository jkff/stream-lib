...
public class SparseIntArray
{
    ...
    public SparseIntArray(int capacity, int itemsPerBlock, int initialBlockCapacity) {
        ...
        this.blocks = new int[capacity/itemsPerBlock+1][];
        ...
        if (p.present) {
            ...
            if (value != 0) {
                ...
                if (size - 1 <= block.length / 4) {
                    // Shrink block 2x
                    int[] newBlock = Arrays.copyOf(block, block.length/2);
            ...
            if (block == null) {
                ...
                block = blocks[whichBlock] = new int[initialBlockCapacity];
            }
            int size = blockSizes[whichBlock];
    ...
    public int bytesUsed() {
        ...
        for (int[] block : blocks) {
            if (block != null) {
                totalBlockSizes += ARRAY_HEADER_SIZE + Integer.SIZE * block.length;
            }
        }
